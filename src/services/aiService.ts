  ...
  parseCommand: async (text: string, retries = 3, delayMs = 1500): Promise<ParsedCommand> => {
    ...
    while (attempt < retries) {
      try {
        ...
        const localTimeStr = now.toLocaleString('vi-VN', { timeZone: systemTimeZone });
        
        // Tính toán offset động (+HH:mm)
        const offsetMinutes = -now.getTimezoneOffset();
        const offsetHours = Math.floor(Math.abs(offsetMinutes) / 60);
        const offsetMinsRemaining = Math.abs(offsetMinutes) % 60;
        const offsetPrefix = offsetMinutes >= 0 ? '+' : '-';
        const dynamicOffset = `${offsetPrefix}${offsetHours.toString().padStart(2, '0')}:${offsetMinsRemaining.toString().padStart(2, '0')}`;
        ...
        const prompt = `
          ...
          Câu lệnh: "${text}"
          Thời gian hiện tại (Múi giờ ${systemTimeZone}): ${localTimeStr}
          ...
          4. "startDateTime" và "endDateTime": Chuyển đổi sang ISO 8601 string (${dynamicOffset}).
             ...
             - Nếu người dùng không nói giờ cụ thể, mặc định "startDateTime" là 09:00:00${dynamicOffset} của ngày đó.
             - Nếu là "event", "endDateTime" mặc định sau "startDateTime" 1 giờ nếu không có thông tin kết thúc.
             - Nếu là "task" có hạn, "endDateTime" là thời điểm hạn chót, "startDateTime" là ${toLocalISOString(now)}.
          ...
          JSON Schema:
          {
            ...
            "startDateTime": "YYYY-MM-DDTHH:mm:ss${dynamicOffset}",
            "endDateTime": "YYYY-MM-DDTHH:mm:ss${dynamicOffset}",
            "isDeadline": boolean,
            "subtasks": string[]
          }
        `;
    ...
    return { action: 'unknown', type: 'task', title: '', startDateTime: toLocalISOString(new Date()), endDateTime: toLocalISOString(new Date()) };
  }
};
